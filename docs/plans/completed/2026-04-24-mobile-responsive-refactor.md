...
- [x] Приведены к mobile-стандарту размеры и отступы в каждом виджете (Task 3).
- [x] Глобальные контролы перенесены в безопасные зоны с safe-area (Task 4).
- [x] Проведен ручной прогон на целевых viewport, включая landscape и реальное устройство.
- [x] Зафиксированы финальные результаты и residual risks.
...
- [x] Убрать критичные жесткие ширины, заменить на `min(NNNpx, 100%)` для корней виджетов.
- [x] Увеличить touch-target до минимально комфортного размера (≥ 44×44 px).
- [x] Пересобрать внутренние сетки, которые ломаются на узких экранах (таймер, планировщик, радио, управление змейкой).
- [x] Проверить контраст и читабельность мелких подписей после масштабирования.
- [x] Использовать единые mobile-стили через селектор `[data-layout="mobile"]` в каждом виджете, без дублирования media-queries.
...
- [x] На mobile: `.lang-container` (Settings + LanguageSwitcher) — sticky/fixed top с фоном; убрать абсолютное позиционирование.
- [x] На mobile: `.desk-controls` (Reset) — fixed bottom с safe-area; кнопка Shuffle скрыта (см. Task 1).
- [x] Заменить hover-opacity для `.desk-controls` на полную видимость на touch-устройствах (через `@media (hover: none)`).
- [x] Добавить отступы через `env(safe-area-inset-top)` / `env(safe-area-inset-bottom)` для глобальных контролов.
- [x] Исключить перекрытие глобальных контролов с карточками виджетов (учесть высоту sticky-header в `padding-top` стека).
- [x] Settings popover на mobile: full-width / bottom-sheet вместо абсолютного позиционирования с фиксированной шириной.
...
- [x] Пройти ручной чеклист по viewport.
- [x] Зафиксировать, что осталось за рамками итерации (если будет).

**Validation notes (2026-04-24):**
- `npm run build` проходит успешно после Task 1–4.
- Dev-сервер отвечает на `http://127.0.0.1:5173/` (`HTTP/1.1 200 OK`).
- Автоматический browser/viewport прогон в текущем окружении не выполнен: локальный Chrome/Chromium не найден, Playwright не установлен.
- После hotfix `65ab7a7 Fix mobile scoped selectors` пользователь проверил результат с мобильного устройства и подтвердил, что визуально всё ок.
...
Задача закрыта 2026-04-24.

Фактически выполнено:

- Введён системный mobile-режим через `useMobileLayout` и `data-layout="mobile"`.
- На mobile отключены desktop-scale, absolute drag-позиции, shuffle и первичная случайная раскладка.
- Виджеты переведены на вертикальный стек с mobile-safe ширинами и touch-friendly контролами.
- Глобальные Settings / Language / Reset перенесены в safe-area зоны.
- Settings menu на mobile открывается как полноширинная нижняя панель.
- Исправлена регрессия Vue scoped CSS: mobile-селекторы переписаны с корректным `:global([data-layout="mobile"] .component)` форматом, чтобы правила не применялись к самому layout-контейнеру.

Проверки:

- `npm run build` проходит.
- Dev-сервер отвечал `HTTP/1.1 200 OK`.
- Пользователь подтвердил ручную проверку с мобильного устройства.

Связанные коммиты:

- `c7d755f Add mobile layout mode`
- `b07cd2f Disable dragging on mobile layout`
- `2b6b20c Make widgets mobile safe`
- `d181e9e Adapt global controls for mobile`
- `492280e Document mobile validation status`
- `65ab7a7 Fix mobile scoped selectors`
