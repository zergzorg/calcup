...
- 30 ready-калькуляторов в `src/data/calculators.ts`;
- 44 `soon`-карточки для будущего наполнения;
...
| Строительство | `/construction` | wallpaper, paint, tile, laminate, floor-screed, brick, drywall | blocks, putty, insulation, concrete, strip-foundation, slab-foundation, rebar, roof, stairs, excavation, lighting, ventilation, snow-load, wind-load |
