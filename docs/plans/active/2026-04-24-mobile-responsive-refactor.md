...
- [ ] Drag отключён централизованно в `useDraggable.ts` (Task 2).
...
- [ ] В `onMouseDown` / `onTouchStart`: ранний `return`, если `isMobileLayout.value === true`.
- [ ] Не сохранять/не загружать `position` из localStorage на mobile (чтобы при возврате на desktop геометрия восстановилась корректно). Альтернатива: оставить чтение из localStorage, но игнорировать применение `position` в mobile-стилях через CSS (`position: static !important` для `[data-layout="mobile"]`).
- [ ] Проверить, что `e.preventDefault()` в `onTouchStart` НЕ вызывается на mobile — иначе блокирует нативный скролл и tap-события.
- [ ] Сохранить текущую механику drag на desktop без функциональных изменений.
