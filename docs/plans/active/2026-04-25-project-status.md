...
## Сделано (Фаза 5.52) — 2026-04-26

Реализован калькулятор `/animals/dog-pregnancy`.

### Реализовано

- Создан feature-модуль `src/features/dog-pregnancy-calculator/`.
- Добавлен расчёт:
  - средней даты родов как выбранная дата + 63 дня;
  - окна щенения 57-65 дней от даты вязки;
  - более узкого окна 62-64 дня от подтверждённой овуляции;
  - дня от выбранной даты, дней до средней даты и этапа timeline.
- Добавлены режимы: дата вязки и дата овуляции.
- Добавлен animal-care warning-note с признаками, при которых нужна срочная ветеринарная помощь.
- Добавлены RU/EN локали `dogPregnancy.*`.
- Registry-запись `dog-pregnancy` добавлена в `ready`.
- В sitemap добавлен `/animals/dog-pregnancy/`.
- README обновлён до 67 ready / 12 soon.

### Проверки

- `npm run test -- dog-pregnancy registry` — OK, 2 files / 11 tests.
- `npm run test` — OK, 67 files / 533 tests.
- `npm run type-check` — OK.
- `npm run build` — OK, Vite SSG rendered 93 pages.
- Static smoke:
  - `/animals/dog-pregnancy/` title, canonical со slash, `index,follow`;
  - `/animals/dog-pregnancy/` есть в sitemap.
- Mobile smoke:
  - `/animals/dog-pregnancy/` 430px full-page screenshot без overflow и наложений.

---

## Сделано (Фаза 5.51) — 2026-04-26

Реализован калькулятор `/animals/cat-calorie`.

### Реализовано

- Создан feature-модуль `src/features/cat-calorie-calculator/`.
- Добавлен расчёт:
  - RER по формуле `70 × weight^0.75`;
  - дневной энергии через множитель профиля;
  - калорий основного корма с вычетом доли лакомств;
  - граммов корма в день и порции при трёх кормлениях.
...
- Добавлены RU/EN локали `catCalorie.*`.
- Registry-запись `cat-calorie` добавлена в `ready`.
- В sitemap добавлен `/animals/cat-calorie/`.
- README обновлён до 66 ready / 12 soon.

### Проверки

- `npm run test -- cat-calorie registry` — OK, 2 files / 8 tests.
- `npm run test` — OK, 66 files / 525 tests.
- `npm run type-check` — OK.
- `npm run build` — OK, Vite SSG rendered 92 pages.
- Static smoke:
  - `/animals/cat-calorie/` title, canonical со slash, `index,follow`;
  - `/animals/cat-calorie/` есть в sitemap.
- Mobile smoke:
  - `/animals/cat-calorie/` 430px full-page screenshot без overflow и наложений.
...
## Сделано (Фаза 5.29) — 2026-04-26

Реализован конвертер `/clothing/clothing-size`.
...
- Добавлен расчёт:
  - ориентировочной таблицы INT/RU/EU/US/UK для XS-XXL;
  - базовых диапазонов мерок груди, талии и бёдер;
  - результата без брендовых обещаний.
- Добавлен предупреждающий блок про различия брендов, посадок и категорий изделий.
- Добавлены RU/EN локали `clothingSize.*`.
- Registry-запись `clothing-size` переведена в `ready`.
- В sitemap добавлен `/clothing/clothing-size/`.
...
- `npm run build` — OK, `vite-ssg` отрендерил 87 страниц.
- Static smoke — OK:
  - `/clothing/clothing-size/` title, canonical со slash, `index,follow`;
  - `/clothing/clothing-size/` есть в sitemap.
- Mobile smoke — OK:
  - `/clothing/clothing-size/` 430px full-page screenshot без overflow и наложений.
