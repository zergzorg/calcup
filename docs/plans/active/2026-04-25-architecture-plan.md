...
## Актуальный статус на 2026-04-25

Базовая архитектура агрегатора уже реализована:

- Vue Router / route registry работают от `src/data/calculators.ts` и `src/data/categories.ts`.
- В каталоге 10 категорий: финансы, математика, строительство, здоровье, дата/время, транспорт, спорт, одежда/обувь, конвертеры, быт.
- В реестре 16 ready-калькуляторов и 58 `soon`-карточек.
- `vite-ssg` и GitHub Pages fallback уже используются в build pipeline.
- `/workspace` больше не является главной концепцией сайта, а живёт как отдельный noindex-инструмент.
- Следующая архитектурная задача: не расширять роутинг вручную, а стабилизировать генерацию sitemap/route indexes вокруг статусов `ready`, `soon`, `planned`.
