...
`NEW_PLAN_DEV.MD` задаёт стратегию: развивать Calcup не как хаотичный каталог, а как набор SEO-кластеров и пользовательских сценариев. На 2026-04-26 в коде уже есть 13 категорий, 113 ready-калькуляторов и 8 soon-карточек. Сильнее всего заполнены строительство, финансы, математика и информатика; главные открытые зоны — animal P1 growth/size, clothing soon-карточки, аккуратные строительные инженерные расчёты, sport expansion и платформенные SEO-хабы.
...
|---|---:|---:|---|
| finance | 13 | 0 | База сильная: кредиты, ипотека, вклады, НДС, планирование и фриланс-сценарии. |
| math | 15 | 0 | P1 учебный кластер закрыт; дальше полезнее education/GPA или подкатегории. |
| health | 5 | 0 | Хорошо для MVP, нужны аккуратные спортивно-оздоровительные расширения. |
| convert | 8 | 0 | Валюты закрыты manual-rate MVP; следующий шаг — UX/SEO вокруг популярных пар. |
| construction | 16 | 5 | Сильный кластер; оставшиеся soon — нормы/инженерка с повышенным риском. |
...
| work | 9 | 0 | Кластер налогов и работы закрыт P1-волной; нужен аудит актуальности и перелинковки. |
| informatics | 17 | 0 | Раздел выделен отдельно от tools; старые `/tools/*` маршруты удалены. |
...
- `budget`: доходы, расходы, долги, накопления, свободный остаток — completed 2026-04-26;
- `inflation`: покупательная способность и реальная доходность — completed 2026-04-26;
- `tax-deduction`: вычеты лечение/обучение/ИИС/жильё — completed 2026-04-26;
- `debt-load`: долговая нагрузка — completed 2026-04-26;
- `rent-vs-buy`: аренда или покупка — completed 2026-04-26.
...
- First wave published as ready routes under `/math/*` through `math-education-calculators`.
- Covered by unit tests for geometry, solids, Pythagorean theorem, triangle, quadratic equations, GCD/LCM, primes, decimal fractions, statistics and combinatorics.
...
1. `dog-size` — следующий animal P1, хороший рост нового раздела без нормативного риска.
2. `cat-growth` — парный animal P1 после dog-size, усиливает кошачий кластер.
3. `sneaker-size` — закрыть clothing soon с умеренным риском и понятным спросом.
4. `jeans-size` — второй clothing soon, полезен после общей одежды/обуви.
5. `one-rep-max` — сильный sport P0/P1, простая формула и высокая польза.
6. `gpa` / `grade` — открыть education-подкатегорию или продолжить учебный блок.
7. `lighting` — low/medium construction soon, но с явным справочным scope.
8. `ventilation` — construction soon, только как бытовой air-change estimate.
9. `car-ownership-cost` — транспортный кластер с сильной практической пользой.
10. `recipe-scale` / `dish-cost` — бытовой food/recipe сценарий, низкий риск и хороший evergreen.
