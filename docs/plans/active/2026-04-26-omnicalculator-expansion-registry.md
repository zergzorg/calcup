...
На 2026-04-26 в `src/data/calculators.ts` есть 11 категорий и 67 ready-калькуляторов. Уже закрыты базовые финансы, математика, здоровье, конвертеры, строительство, транспорт, спорт, животные, одежда, даты и быт.

Крупные пробелы:

- категория `animals` создана калькуляторами `dog-age`, `cat-age`, `dog-food`, `cat-calorie` и `dog-pregnancy`, но ей нужен более широкий pet-care кластер;
- нет категории `food`;
