...
Calcup — каталог бесплатных онлайн-калькуляторов и конвертеров. Главная концепция проекта больше не ограничивается рабочим столом продуктивности: сайт развивается как SEO-friendly агрегатор быстрых расчётов для финансов, математики, здоровья, строительства, транспорта, животных, дат, конвертеров, спорта, одежды и бытовых задач.
...
- 71 готовый калькулятор.
- 9 запланированных `soon`-карточек для расширения каталога.
...
- `/finance/` — кредит, ипотека, вклад, сложный процент, рефинансирование, НДС, зарплата, стоимость проекта.
- `/math/` — проценты, дроби, среднее значение, пропорции и уравнения.
- `/health/` — ИМТ, суточная норма калорий, ориентир идеального веса, процент жира и срок беременности.
- `/convert/` — длина, температура, вес, площадь, объём, скорость и объём данных.
- `/construction/` — обои, краска, плитка, ламинат, стяжка пола, кирпич, газоблок, гипсокартон, шпатлёвка, утеплитель, бетон, ленточный фундамент, плитный фундамент, арматура, кровля и лестница.
...
- `/animals/` — возраст собаки и кошки, дневной корм, калории и срок беременности питомцев.
...
- `/finance/mortgage/` — ипотека, первоначальный взнос, платёж и переплата.
- `/finance/deposit/` — вклад, доход, капитализация и итоговая сумма.
- `/finance/compound-interest/` — сложный процент, пополнения, доход и рост капитала.
- `/finance/refinance/` — сравнение старого и нового кредита, выгода и окупаемость.
...
- `/health/calorie/` — базовый обмен, дневной расход энергии и ориентир калорий.
- `/health/ideal-weight/` — ориентир веса по росту, полу и нескольким формулам.
- `/health/body-fat/` — ориентировочный процент жира по окружностям тела.
- `/health/pregnancy-due-date/` — ориентировочная дата родов, срок беременности и триместр.
...
- `/construction/slab-foundation/` — бетон, песчано-щебёночная подушка, арматурная сетка и стоимость для плитного фундамента.
- `/construction/rebar/` — прутки, длина, вес и стоимость арматуры для прямоугольной сетки.
- `/construction/roof/` — площадь скатов, листы кровли, нахлёсты, запас и стоимость.
- `/construction/stairs/` — ступени, подступёнок, проступь, угол лестницы и закупка.
...
- `/animals/dog-age/` — возраст собаки в человеческих годах с учётом размера.
- `/animals/cat-age/` — возраст кошки в человеческих годах и жизненный этап.
- `/animals/dog-food/` — дневные калории и граммы корма для собаки.
- `/animals/cat-calorie/` — дневные калории и граммы корма для кошки.
- `/animals/dog-pregnancy/` — ориентировочная дата родов и окно щенения у собаки.
- `/animals/cat-pregnancy/` — ориентировочная дата родов и окно окота у кошки.
