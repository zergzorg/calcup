...
- 38 готовых калькуляторов.
- 36 запланированных `soon`-карточек для расширения каталога.
...
- `/construction/` — обои, краска, плитка, ламинат, стяжка пола, кирпич, газоблок, гипсокартон, шпатлёвка, утеплитель, бетон.
- `/transport/` — расход топлива, стоимость поездки, цена топлива, средняя скорость, запас хода EV.
- `/sport/` — темп, скорость, дистанция, время бега, сплиты и пульсовые зоны.
- `/clothing/` — конвертер размера обуви.
- `/datetime/` — разница между датами, возраст, рабочие дни, длительность времени.
- `/everyday/` — чаевые, скидка, выгодная покупка.

Новое направление `/clothing/` уже открыто первым готовым конвертером размера обуви; следующие size-инструменты остаются в roadmap.
